...
<head>
    ...
    <script>
        ...
        /*需检查   开始:用户名校验*/
        function fuserNameRegister() {
            var userNameRegister = $("#userNameRegister").val();//获取userName文本输入框的值
            ...
            if (userNameRegister.length == 0) {
                ...
                xianshi.html("账户不能为空");
            } else if (!/^[a-zA-Z0-9_]{6,10}$/.test(userNameRegister)) {
                zhuangtai = false;
                xianshi.css("color", "red");
                xianshi.html("账户长度必须6-10位的英文、数字、下划线组成");
            } else {
                $.post("${pageContext.request.contextPath}/userInfo/userNameRegister.do", {"userName": userNameRegister}, function (data) {
                    if (data.userExsit) {
                        zhuangtai = false;
                        //用户名不能使用
                        xianshi.css("color", "red");
        ...
        $(function () {
            $("#fsubmit").click(function () {
                if (fuserNameRegister() && fuserPassRegister() && femailRegister()) {
                    $("#regform").submit();
                } else if (!fuserNameRegister()) {
                    alert("注册失败:账户不可用,请重新输入!")
                } else if (!fuserPassRegister()) {
                    alert("注册失败:密码不可用,请重新输入!")
                } else if (!femailRegister()) {
                    alert("注册失败:邮箱不可用,请重新输入!")
                }
            })
...
<div class="hm-body hm-body-bgc">
    <div class="hm-inner">
        <div class="reg-box">
            ...
            <div class="reg-info">
                <form id="regform" action=${pageContext.request.contextPath}"/userInfo/wuxinghui.do" method="post">
                    <ul>
                        <li>
                            <div class="reg-l">
                            ...
                            <div class="reg-c">
                                <input type="text" id="userNameRegister" name="userName" class="txt"
                                       onblur="fuserNameRegister()"/>
                            </div>
